...
export default function PayablesSection() {
  ...
  const [form, setForm] = useState({ supplierName: '', concept: '', amount: '', dueDate: '' })
  const [payForm, setPayForm] = useState({ amount: '', method: 'transferencia', date: new Date().toISOString().slice(0, 10) })
  const [error, setError] = useState('')
  const [hoveredRow, setHoveredRow] = useState(null)

  const load = async () => { setItems(await service.getPayables()) }
  useEffect(() => { load() }, [])

  const today = new Date().toISOString().slice(0, 10)
  const totalPending = items.filter(p => p.status !== 'paid').reduce((s, p) => s + (p.amount || 0) - (p.paid || 0), 0)
  const overdueToday = items.filter(p => p.status !== 'paid' && p.dueDate && p.dueDate <= today).reduce((s, p) => s + (p.amount || 0) - (p.paid || 0), 0)
  const paidThisMonth = items.filter(p => p.status === 'paid').reduce((s, p) => s + (p.paid || 0), 0)

  const filtered = items.filter(p => {
    if (filter === 'pending') return p.status === 'pending'
    if (filter === 'overdue') return p.status !== 'paid' && p.dueDate && p.dueDate <= today
  ...
  const handleCreate = async () => {
    setError('')
    try {
      if (!form.supplierName || !form.amount) { setError('Proveedor y monto son requeridos'); return }
      await service.createPayable({ ...form, amount: Number(form.amount) })
      setForm({ supplierName: '', concept: '', amount: '', dueDate: '' })
      setShowModal(false)
      load()
  ...
  const handlePay = async () => {
    setError('')
    try {
      await service.payPayable(showPayModal.id, { ...payForm, amount: Number(payForm.amount) })
      setShowPayModal(null)
      setPayForm({ amount: '', method: 'transferencia', date: new Date().toISOString().slice(0, 10) })
      load()
    } catch (e) { setError(e.message) }
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
        ...
        <div style={cardStyle}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Pagado Este Mes</div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            {['Proveedor', 'Concepto', 'Monto', 'Pagado', 'Pendiente', 'Vence', 'Estado', 'Acción'].map(h => (
          ...
          <tbody>
            <AnimatePresence mode="popLayout">
              {filtered.map((p, i) => {
                ...
                return (
                  <motion.tr key={p.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                    ...
                    <td style={{ ...tdStyle, fontWeight: 600, color: 'var(--text-primary)' }}>{p.supplierName}</td>
                    <td style={tdStyle}>{p.concept || '—'}</td>
                    <td style={{ ...tdStyle, fontWeight: 600 }}>{formatMoney(p.amount)}</td>
                    <td style={{ ...tdStyle, color: '#10b981' }}>{formatMoney(p.paid || 0)}</td>
                    <td style={{ ...tdStyle, fontWeight: 600, color: '#ef4444' }}>{formatMoney((p.amount || 0) - (p.paid || 0))}</td>
                    <td style={tdStyle}>{formatDate(p.dueDate)}</td>
                    <td style={tdStyle}>
                      <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: ac.bg, color: ac.color }}>
                        {p.status === 'paid' ? 'Pagado' : days > 0 ? `${days}d vencido` : 'Pendiente'}
                      </span>
                    </td>
                    <td style={tdStyle}>
                      {p.status !== 'paid' && (
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowPayModal(p)}
                          style={{ padding: '5px 12px', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 6, color: '#6366f1', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                          Pagar
                        </motion.button>
                      )}
                    </td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} style={{ padding: 48, textAlign: 'center' }}>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: 14 }}>No hay cuentas por pagar</div>
              </td></tr>
            )}
          </tbody>
        </table>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ color: 'var(--text-primary)', fontSize: 16, fontWeight: 700, margin: 0 }}>Nueva Cuenta por Pagar</h3>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              <div><label style={labelStyle}>Proveedor</label><input value={form.supplierName} onChange={e => setForm({ ...form, supplierName: e.target.value })} placeholder="Nombre del proveedor" style={inputStyle} /></div>
      ...
      {showPayModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ color: 'var(--text-primary)', fontSize: 16, fontWeight: 700, margin: 0 }}>Registrar Pago — {showPayModal.supplierName}</h3>
            ...
            {error && <div style={{ padding: '10px 14px', background: 'rgba(220,38,38,0.1)', color: '#ef4444', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{error}</div>}
            <div style={{ display: 'grid', gap: 12 }}>
              <div><label style={labelStyle}>Monto a pagar</label><input type="number" min={0} step="0.01" value={payForm.amount} onChange={e => setPayForm({ ...payForm, amount: e.target.value })} style={inputStyle} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 }}>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={handlePay} style={btnPrimary}>Registrar Pago</motion.button>
            </div>
          </motion.div>
        </div>
      )}
