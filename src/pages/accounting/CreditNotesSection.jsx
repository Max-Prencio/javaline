...
export default function CreditNotesSection() {
  ...
  const load = async () => { setItems(await service.getCreditNotes()) }
  useEffect(() => { load() }, [])
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await service.createCreditNote({ ...form, amount: Number(form.amount), tax: Number(form.tax) || 0, date: new Date().toISOString().slice(0, 10) })
    ...
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => setShowModal(true)} style={btnPrimary}>
          <FiPlus size={16} /> Nueva Nota de Crédito
        </motion.button>
      </div>
          ...
          <tbody>
            ...
            {items.length === 0 && (
              <tr><td colSpan={8} style={{ padding: 48, textAlign: 'center' }}>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: 14 }}>No hay notas de crédito registradas</div>
              </td></tr>
            )}
          </tbody>
        </table>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ color: 'var(--text-primary)', fontSize: 16, fontWeight: 700, margin: 0 }}>Nueva Nota de Crédito</h3>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div><label style={labelStyle}>Concepto</label><input value={form.concept} onChange={e => setForm({ ...form, concept: e.target.value })} placeholder="Motivo de la nota de crédito" style={inputStyle} /></div>
